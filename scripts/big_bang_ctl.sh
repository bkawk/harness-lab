...
LLM_PROPOSAL_ENABLED="${BIG_BANG_LLM_PROPOSAL_ENABLED:-1}"
...
start_big_bang() {
  ...
  (
    cd "$REPO_DIR"
    nohup env PYTHONPATH=src HARNESS_LAB_SCIENCE_TIME_BUDGET_SECONDS="$SCIENCE_TIME_BUDGET_SECONDS" HARNESS_LAB_SCIENCE_EVAL_RESERVE_SECONDS="$SCIENCE_EVAL_RESERVE_SECONDS" HARNESS_LAB_RUNNER_STALE_SECONDS="$RUNNER_STALE_SECONDS" HARNESS_LAB_LLM_PROPOSAL_ENABLED="$LLM_PROPOSAL_ENABLED" "$PYTHON_BIN" scripts/big_bang.py "${args[@]}" >>"$LOG_FILE" 2>&1 &
    echo $! >"$PID_FILE"
  )

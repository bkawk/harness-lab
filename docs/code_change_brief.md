...
## Decision State
- `iterate`
- `science_train` is already the active recent seam with outcomes ['dead_end', 'keeper', 'dead_end', 'keeper'], so keep iterating on that line rather than issuing a brand-new brief.

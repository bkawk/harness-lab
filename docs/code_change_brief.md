...
## Decision State
- `iterate`
- `science_loss` is already the active recent seam with outcomes ['audit_blocked', 'dead_end', 'audit_blocked'], so keep iterating on that line rather than issuing a brand-new brief.

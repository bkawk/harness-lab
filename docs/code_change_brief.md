...
## Decision State
- `iterate`
- `science_train` is already the active recent seam with outcomes ['keeper', 'dead_end', 'dead_end', 'dead_end'], so keep iterating on that line rather than issuing a brand-new brief.

...
def _llm_proposal_prompt(
    *,
    ...
    return (
        ...
        "When the live pressure is VRAM headroom, strongly prefer science_train or science_model capacity moves over science_loss.\n"
        "Start with science_train -> {batch_size: 3, eval_batch_size: 3}; if a model-side capacity move is more appropriate, use science_model -> {hidden_dim: 160, global_dim: 256}.\n"
        "When the live pressure is hard transfer regression, prefer paired science_loss moves that strengthen transfer-sensitive structure, such as science_loss -> {boundary_loss_weight: 0.12, instance_margin: 0.38} or science_loss -> {instance_loss_weight: 0.06, instance_margin: 0.38}.\n"
        "Good examples: science_model -> {hidden_dim: 160}; science_model -> {hidden_dim: 160, global_dim: 256}; science_train -> {batch_size: 3, eval_batch_size: 3}; science_loss -> {boundary_loss_weight: 0.12, instance_margin: 0.38}; science_eval -> {transfer_smoke_max_gap: 0.025}.\n"
